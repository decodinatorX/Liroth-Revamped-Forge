package com.decodinator.liroth.portal_junk.portal.frame;
...
public class FlatPortalAreaHelper extends PortalFrameTester {
    ...
    public void lightPortal(Block frameBlock) {
        ...
        BlockState blockState = CustomPortalHelper.blockWithAxis(link != null ? link.getPortalBlock().defaultBlockState() : Liroth.getDefaultPortalBlock().defaultBlockState(), Direction.Axis.Y);
    ...
    public void createPortal(Level world, BlockPos pos, BlockState frameBlock, Direction.Axis axis) {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            fillAirAroundPortal(world, pos.relative(Direction.Axis.X, i).below());
            fillAirAroundPortal(world, pos.relative(Direction.Axis.X, i).relative(Direction.Axis.Z, 1).below());
