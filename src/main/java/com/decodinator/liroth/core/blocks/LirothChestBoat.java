...
public class LirothChestBoat extends LirothBoat implements HasCustomInventoryScreen, ContainerEntity {
   ...
   public LirothChestBoat(Level p_219872_, double p_219873_, double p_219874_, double p_219875_) {
      this(LirothEntities.LIROTH_CHEST_BOAT.get(), p_219872_);
   ...
   public void destroy(DamageSource p_219892_) {
      ...
      this.chestVehicleDestroyed(p_219892_, this.getLevel(), this);
   }

   public void remove(Entity.RemovalReason p_219894_) {
      if (!this.getLevel().isClientSide && p_219894_.shouldDestroy()) {
   ...
   public InteractionResult interact(Player p_219898_, InteractionHand p_219899_) {
      return this.canAddPassenger(p_219898_) && !p_219898_.isSecondaryUseActive() ? super.interact(p_219898_, p_219899_) : this.m_268996_(p_219898_);
   }

   public void openCustomInventoryScreen(Player p_219906_) {
      p_219906_.openMenu(this);
      if (!p_219906_.getLevel().isClientSide) {
   ...
   public Item getDropItem() {
      ...
      switch (this.getLirothBoatType()) {
         case DAMNATION:
            ...
            break;
         case SPICED:
            item = LirothItems.CHEST_SPICED_BOAT.get();
            break;
         case PIER:
            item = LirothItems.CHEST_PIER_BOAT.get();
            break;
         case JAPZ:
            item = LirothItems.CHEST_JAPZ_BOAT.get();
            break;
         case KOOLAW:
            item = LirothItems.CHEST_KOOLAW_BOAT.get();
            break;
         case PETRIFIED:
            item = LirothItems.CHEST_PETRIFIED_BOAT.get();
            break;
         default:
            item = LirothItems.CHEST_LIROTH_BOAT.get();
      }
