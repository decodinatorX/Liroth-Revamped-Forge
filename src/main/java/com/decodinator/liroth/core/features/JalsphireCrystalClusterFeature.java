...
extends Feature<DripstoneClusterConfiguration> {
    public JalsphireCrystalClusterFeature(Codec<DripstoneClusterConfiguration> codec) {
        super(codec);
    }
    ...
    private void generate(WorldGenLevel world, RandomSource random, BlockPos pos, int localX, int localZ, float wetness, double dripstoneChance, int height, float density, DripstoneClusterConfiguration config) {
        ...
        Column caveSurface;
        boolean bl;
        Optional<Column> optional = Column.scan(world, pos, config.floorToCeilingSearchRange, JalsphireCrystalHelper::canGenerate, JalsphireCrystalHelper::canReplaceOrLava);
        ...
        if (optionalInt.isPresent()) {
        	JalsphireCrystalHelper.generatePointedDripstone(world, pos.atY(optionalInt.getAsInt() - 1), Direction.DOWN, m, bl4);
        }
        if (optionalInt3.isPresent()) {
        	JalsphireCrystalHelper.generatePointedDripstone(world, pos.atY(optionalInt3.getAsInt() + 1), Direction.UP, t, bl4);
        }
    }
    ...
    private boolean canWaterSpawn(WorldGenLevel world, BlockPos pos) {
        ...
        if (blockState.is(Blocks.WATER) || blockState.is(LirothBlocks.JALSPHIRE_CRYSTAL_BLOCK.get()) || blockState.is(LirothBlocks.POINTED_JALSPHIRE_CRYSTAL.get())) {
            return false;
        }
